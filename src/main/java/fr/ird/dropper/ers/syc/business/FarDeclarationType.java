...
public class FarDeclarationType extends EvenementDePecheImpl {
    ...
    public void addToLog(LogDeclarationType log) {
        if (log.getFAR()== null) {
            log.setFAR(new ArrayList<FarDeclarationType>());
        }
        log.getFAR().add(this);
    }
    ...
    public List<RasDeclarationType> getRasList() {
        List<RasDeclarationType> listRas = new ArrayList<RasDeclarationType>();
        if (getRAS() != null) {
            listRas.add(getRAS());
        }
        if (getSPE() == null) {
